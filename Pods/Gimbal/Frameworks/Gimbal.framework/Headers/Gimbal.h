...
#import <Gimbal/GMBLDeviceAttributesManager.h>
...
/*!
 ...
 @discussion This also prompts the user for access to their location. By default the user will be prompted with the options "Always", "While in use" or "Never". You can manage this request on your own in the options parameter by setting the key "MANAGE_PERMISSIONS" with a value of NO.
 */
+ (void)setAPIKey:(NSString *)APIKey options:(NSDictionary *)options;

/*!
 Starts all the Gimbal services which includes the generation of events based on the user's location and proximity to
 geofences and beacons, receive communications associated to place entry and exit events and generation of established
 locations based on the user's locations. It's a method for starting the GMBLPlaceManager, GMBLCommunicationManager and
